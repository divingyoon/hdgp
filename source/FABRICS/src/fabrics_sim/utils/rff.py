...
class RFF:
    ...
    def to_features_jacobian(self, x):
        pass
    ...
    def __init__(self, n_feat, d, sigma, cos_only=False, quasi_random=True,
        ...
        if cos_only:  # cos only features
            ...
            self.to_features_jacobian = self._to_cos_sin_features_jacobian
        self.freqs = torch.from_numpy(self.freqs).float().to(self.device)

    @staticmethod
    def draw_freqs(rff_kernel, m, d, quasi_random):
        if quasi_random:
            # NOTE: the perms only works for d up to 100, so it's uncommented
            # and the Halton sequence is constructed from d instead.
            # NOTE: d is the number of inputs that are turned into RFFs
            assert(d <= 100), "dim of inputs to RRFs beyond 100. Check rff.py file for more info."
            perms = ghalton.EA_PERMS[:d]
            sequencer = ghalton.GeneralizedHalton(perms)
            # NOTE: one would potentially use the below for d > 100
    ...
    def _to_cos_sin_features_jacobian(self, x, sigma=None):
        sigma = (self.sigma if sigma is None else sigma)
        coeff = self.freqs / sigma
        inner = torch.matmul(x, coeff.T)
