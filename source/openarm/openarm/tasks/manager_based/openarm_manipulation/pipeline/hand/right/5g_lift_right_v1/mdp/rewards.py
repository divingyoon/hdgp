...
def _get_fingertip_positions(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    tip_info = [
        ("tesollo_right_rl_dg_1_4", "y", -0.0363),  # thumb
        ("tesollo_right_rl_dg_2_4", "z", 0.0255),   # index
        ("tesollo_right_rl_dg_3_4", "z", 0.0255),   # middle
        ("tesollo_right_rl_dg_4_4", "z", 0.0255),   # ring
        ("tesollo_right_rl_dg_5_4", "z", 0.0363),   # pinky
    ]
...
def _thumb_opposition_reward(
    ...
    thumb_tip = _get_fingertip_world_position(env, "tesollo_right_rl_dg_1_4", "y", -0.0363)
    if thumb_tip is None:
        return torch.zeros(env.num_envs, device=env.device)

    others = []
    for body_name, axis, offset in [
        ("tesollo_right_rl_dg_2_4", "z", 0.0255),
        ("tesollo_right_rl_dg_3_4", "z", 0.0255),
        ("tesollo_right_rl_dg_4_4", "z", 0.0255),
        ("tesollo_right_rl_dg_5_4", "z", 0.0363),
    ]:
...
def _maybe_log_grasp_quality(
    ...
    obj: RigidObject = env.scene[object_cfg.name]
    current_xy = obj.data.root_pos_w[:, :2]
    initial_xy = _get_episode_initial_object_xy(env, obj, "_cup_initial_xy_w_right")
...
def _lift_trigger(
    ...
    initial_z = _get_episode_initial_object_z(env, obj, "_cup_initial_z_w_right")
...
def object_ee_distance(
    ...
    target_pos_w = _compute_grasp_target_pos_w(
        ...
        ee_pos_w,
        use_dynamic_z=True,
        dynamic_z_state_attr="_reach_dynamic_z_prev_right",
    )
...
def _is_reaching_stably_complete(
    ...
    if not hasattr(env, "_reach_latched_right"):
        env._reach_latched_right = torch.zeros(env.num_envs, device=env.device, dtype=torch.bool)

    counter = env._reach_hold_counter_right
    latched = env._reach_latched_right
    ...
    if env._reach_hold_counter_right_last_step != step_count:
        ...
        env._reach_latched_right = latched
...
def _is_grasp_stably_complete(
    ...
    if not hasattr(env, "_grasp_hold_counter_right"):
        env._grasp_hold_counter_right = torch.zeros(env.num_envs, device=env.device, dtype=torch.int64)
    counter = env._grasp_hold_counter_right

    step_count = int(getattr(env, "common_step_counter", -1))
    if not hasattr(env, "_grasp_hold_counter_right_last_step"):
        env._grasp_hold_counter_right_last_step = -2
    if env._grasp_hold_counter_right_last_step != step_count:
        reset_mask = (env.episode_length_buf == 0).squeeze(-1)
        counter[reset_mask] = 0
        counter = torch.where(reached_now_i64 > 0, counter + 1, torch.zeros_like(counter))
        env._grasp_hold_counter_right = counter
        env._grasp_hold_counter_right_last_step = step_count
...
def _is_lifting_sustained(
    ...
    if not hasattr(env, "_lift_hold_counter_right"):
        env._lift_hold_counter_right = torch.zeros(env.num_envs, device=env.device, dtype=torch.int64)
    if not hasattr(env, "_lift_latched_right"):
        env._lift_latched_right = torch.zeros(env.num_envs, device=env.device, dtype=torch.bool)

    counter = env._lift_hold_counter_right
    latched = env._lift_latched_right

    step_count = int(getattr(env, "common_step_counter", -1))
    if not hasattr(env, "_lift_hold_counter_right_last_step"):
        env._lift_hold_counter_right_last_step = -2
    if env._lift_hold_counter_right_last_step != step_count:
        ...
        env._lift_hold_counter_right = counter

        # Once sustained, latch it
        newly_latched = (counter >= hold_steps) & (~latched)
        latched = latched | newly_latched
        env._lift_latched_right = latched

        env._lift_hold_counter_right_last_step = step_count
...
def object_displacement_penalty(
    ...
    initial_pos = _get_episode_initial_object_xy(env, obj, "_cup_initial_xy_w_right")
    ...
    grasp_gate = _grasp_progress_gate(env, object_cfg, eef_link_name="rl_dg_ee")
    penalty = penalty * ((1.0 - gate_mix) + gate_mix * grasp_gate)
    # Disable displacement penalty once grasp is established (μ=1).
    mu_trigger = _grasp_trigger(env, object_cfg, eef_link_name="rl_dg_ee")
...
def finger_contact_reward(
    ...
    _TIPS = [
        ("tesollo_right_rl_dg_1_4", "y", -0.0363),
        ("tesollo_right_rl_dg_2_4", "z", 0.0255),
        ("tesollo_right_rl_dg_3_4", "z", 0.0255),
        ("tesollo_right_rl_dg_4_4", "z", 0.0255),
        ("tesollo_right_rl_dg_5_4", "z", 0.0363),
...
def thumb_grasp_reward(
    ...
    thumb_tip = _get_fingertip_world_position(
        env, "tesollo_right_rl_dg_1_4", "y", -0.0363
    )
    if thumb_tip is None:
        return torch.zeros(env.num_envs, device=env.device)
    ...
    finger2_tip = _get_fingertip_world_position(
        env, "tesollo_right_rl_dg_2_4", "z", 0.0255
    )
    ...
    thumb_contact = _finger_surface_contact_gate(
        env, "tesollo_right_rl_dg_1_4", "y", -0.0363, object_cfg=object_cfg
    )
    grasp_gate = _grasp_progress_gate(env, object_cfg, eef_link_name)
    pinky_contact = _finger_surface_contact_gate(
        env, "tesollo_right_rl_dg_5_4", "z", 0.0363, object_cfg=object_cfg
    )
...
def pinky_grasp_reward(
    ...
    pinky_tip = _get_fingertip_world_position(
        env, "tesollo_right_rl_dg_5_4", "z", 0.0363
    )
...
def finger_tip_to_cup_reward(
    ...
    _TIP_INFO = [
        ("tesollo_right_rl_dg_1_4", "y", -0.0363),  # thumb: Y offset
        ("tesollo_right_rl_dg_2_4", "z", 0.0255),   # index: Z offset
        ("tesollo_right_rl_dg_3_4", "z", 0.0255),   # middle: Z offset
        ("tesollo_right_rl_dg_4_4", "z", 0.0255),   # ring: Z offset
        ("tesollo_right_rl_dg_5_4", "z", 0.0363),   # pinky: Z offset
    ]
...
def finger_wrap_cylinder_reward(
    ...
    tip_info = [
        ("thumb", "tesollo_right_rl_dg_1_4", "y", -0.0363),
        ("index", "tesollo_right_rl_dg_2_4", "z", 0.0255),
        ("middle", "tesollo_right_rl_dg_3_4", "z", 0.0255),
        ("ring", "tesollo_right_rl_dg_4_4", "z", 0.0255),
        ("pinky", "tesollo_right_rl_dg_5_4", "z", 0.0363),
    ]
...
def finger_tip_orientation_reward(
    ...
    _TIP_INFO = [
        ("tesollo_right_rl_dg_1_4", "y", -0.0363, "y"),  # thumb: Y offset, Y normal
        ("tesollo_right_rl_dg_2_4", "z", 0.0255, "x"),   # index: Z offset, X normal
        ("tesollo_right_rl_dg_3_4", "z", 0.0255, "x"),   # middle: Z offset, X normal
        ("tesollo_right_rl_dg_4_4", "z", 0.0255, "x"),   # ring: Z offset, X normal
        ("tesollo_right_rl_dg_5_4", "z", 0.0363, "x"),   # pinky: Z offset, X normal
    ]
...
def synergy_tip_z_reward(
    ...
    body_name = "tesollo_right_rl_dg_2_4"
