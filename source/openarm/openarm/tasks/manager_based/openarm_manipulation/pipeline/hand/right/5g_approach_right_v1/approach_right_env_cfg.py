...
"""Right-only approach environment configuration for 5g hand task.

Scene layout (table/cups) and physics buffers are kept aligned with 2g_grasp_right_v1.
...
@configclass
class ApproachRightSceneCfg(InteractiveSceneCfg):
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        """Policy observations focused on right approach behavior."""
...
class RewardsCfg:
    ...
    cup2_displacement_penalty = RewTerm(
        func=mdp.object_root_displacement_penalty,
        params={"object_cfg": SceneEntityCfg("cup2"), "scale": 1.0},
        weight=-5.0,
    )
...
@configclass
class ApproachRightEnvCfg(ManagerBasedRLEnvCfg):
    """Right-hand cup approach environment with 5g hand."""

    scene: ApproachRightSceneCfg = ApproachRightSceneCfg(num_envs=2048 * 1, env_spacing=2.5)
    ...
    def __post_init__(self):
        ...
        # Keep physics buffer sizing same as 2g_grasp_right_v1.
