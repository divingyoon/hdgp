...
def object_ee_distance(
    ...
    target_pos_w_dynamic = _compute_grasp_target_pos_w(
        ...
        ee_pos_w,
        use_dynamic_z=True,
        dynamic_z_state_attr="_reach_dynamic_z_prev_right_approach",
    )
    target_pos_w_low = _compute_grasp_target_pos_w(env, obj, ee_pos_w, use_dynamic_z=False)
    _maybe_visualize_approach_target(env, target_pos_w_dynamic, obj.data.root_quat_w, marker_attr="_debug_approach_target_right")
    ...
    current_xy = obj.data.root_pos_w[:, :2]
    initial_xy = _get_episode_initial_object_xy(env, obj, "_cup2_initial_xy_w_right_approach")
