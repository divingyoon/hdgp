...
class Grasp5gHoldEnv(ManagerBasedRLEnv):
    """Grasp env with 2g-style inactive-arm action masking."""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.mask_inactive_arm = getattr(self.cfg, "mask_inactive_arm_actions", True)
        try:
            if "objects" in self.scene.keys():
                self.object_bank_size = int(self.scene["objects"].num_objects)
        except Exception:
            self.object_bank_size = int(getattr(self, "object_bank_size", 1))
