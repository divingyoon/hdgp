...
- 액션: 11D = 6D palm pose + 5D PCA
- 리워드: v2 기반 + rl-icub-dexterous-manipulation 이식 항목

v3 이식 사항 (rl-icub-dexterous-manipulation → OpenArm/IsaacLab 변환):
1. delta_contact_reward: 손가락 접촉 수 증가분 보상 (이식: iCub diff_num_contacts())
2. min_fingers_touching_for_success: N개 손가락 접촉 성공 게이트 (이식: min_fingers_touching_object)
3. scale_lift_reward_with_contacts: 접촉 수에 비례한 리프트 보상 (이식: scale_pos_lift_reward_wrt_touching_fingers)
4. num_fingers_touching 관측 (1D, 정규화): 접촉 정보를 policy에 직접 노출
"""
...
class GraspRightEnvCfg(DirectRLEnvCfg):
    ...
    # Fabrics 파라미터
    # -----------------------------------------------------------------------
    # v3: hand는 직접 PD 제어, Fabrics hand_attractor 비활성화
    use_hand_fabric: bool = False
    # 손가락 curl 증분 제어 파라미터 (iCub max_delta_qpos 방식)
    max_delta_hand_q: float = 0.05  # 스텝당 최대 변화량 (rad)
    # Proximity-gated curl: palm이 물체에 충분히 가까워진 뒤에만 손가락 닫기 허용
    curl_gate_enabled: bool = True
    curl_gate_distance: float = 0.10   # 이 거리(m) 이내로 접근해야 curl 허용
    curl_gate_sharpness: float = 40.0  # sigmoid 경계 날카로움 (클수록 단계적)
    ...
    # -----------------------------------------------------------------------
    # v3 추가: rl-icub-dexterous-manipulation 이식 항목
    # -----------------------------------------------------------------------
    # 1. delta_contact_reward: 손가락 접촉 수 변화량 보상/패널티
    #    이식: iCub ICubEnvRefineGrasp.diff_num_contacts() (양방향)
    #    증가 시: delta_contact_weight × delta (양수 보상)
    #    감소 시: delta_contact_decrease_weight × |delta| (음수 패널티, 슬립 억제)
    delta_contact_weight: float = 1.0
    delta_contact_decrease_weight: float = 0.5  # 감소 패널티 (iCub 원본=1.0, 절반으로 시작해 탐색 유지)

    # 2. min_fingers_touching_for_success: 성공 판정에 N개 손가락 접촉 필요
    #    이식: iCub min_fingers_touching_object=5 (→ 5개 손가락 접촉 성공조건)
    #    OpenArm은 5 fingertip × 1 palm = 최대 6, palm 제외 tiponly 사용
    #    기본값 2: 엄지+1 이상 접촉 시 성공 인정 (palm-only 성공 방지)
    min_fingers_touching_for_success: int = 2

    # 3. scale_lift_reward_with_contacts: 접촉 수에 비례한 리프트 보상 스케일링
    #    이식: iCub scale_pos_lift_reward_wrt_touching_fingers
    #    lift_reward *= (num_fingers_touching / num_fingertips)
    #    접촉 없이 들기만 해도 보상 받는 치팅 방지
    scale_lift_reward_with_contacts: bool = True
    ...
    # |F_z| / |F| > cos_threshold → 수직 접촉으로 판정 → 파지 접촉에서 제외
    # 옆면 접근 시 정상 접촉(frame[2]≈0)은 통과, 위에서 누르기(frame[2]≈1.0)는 차단
    exclude_vertical_touches: bool = True
    vertical_contact_cos_threshold: float = 0.707  # 1/√2 ≈ 45° 기준 (iCub 원본과 동일)
    ...
    # 물체 spawn
    # -----------------------------------------------------------------------
    object_spawn_x_center: float = 0.4   # palm 시작 x≈0.46 → 컵을 palm 앞에 배치, +x+y 방향 접근
    object_spawn_y_center: float = 0.0  # palm workspace y∈[-0.50,-0.05] 중앙 근처; -Y 방향 접근
    object_spawn_z: float = 0.38        # visdex 물체 z_min=-0.076 기준 안전 높이
    object_spawn_xy_range: float = 0.00  # ±0.02m 균등 분포
    # 물체 활성 플래그
    ...
    # -----------------------------------------------------------------------
    # 로봇 설정
    # openarm_tesollo_sensor.usd:
    #   - OpenArm bimanual + Teosllo right hand (fingertip contact sensor 내장)
    ...
    #   - rl_dg_X_tip: 각 손가락 말단 sensor link (FixedJoint)
    # -----------------------------------------------------------------------
    robot_cfg: ArticulationCfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            ...
            joint_pos={
                ...
                "rj_dg_1_1": 0.0, "rj_dg_1_2":  -1.5, "rj_dg_1_3": 0.0, "rj_dg_1_4": 0.0,
        ...
        actuators={
            ...
            # Teosllo hand: USD 공식 물성치 적용 (dg5f_right_vs_allegro_joint_comparison.md 기준)
            # _1 joints (abduction/spread): 평균 stiffness≈1.9, damping≈7.5e-4
            "tesollo_hand_abduction": ImplicitActuatorCfg(
                joint_names_expr=["rj_dg_[1-5]_1"],
                stiffness=1.9,
                damping=7.5e-4,
            ),
            # _2 joints (main curl, RL 직접 제어): 평균 stiffness≈0.84, damping≈3.3e-4
            "tesollo_hand_curl": ImplicitActuatorCfg(
                joint_names_expr=["rj_dg_[1-5]_2"],
                stiffness=0.84,
                damping=3.3e-4,
            ),
            # _3 joints (PIP, iCub distal tendon 커플링): 평균 stiffness≈0.43, damping≈1.7e-4
            "tesollo_hand_pip": ImplicitActuatorCfg(
                joint_names_expr=["rj_dg_[1-5]_3"],
                stiffness=0.43,
                damping=1.7e-4,
            ),
            # _4 joints (DIP, iCub distal tendon 커플링): 평균 stiffness≈0.13, damping≈5.1e-5
    ...
    # NOTE:
    #   openarm_tesollo_sensor.usd는 rl_dg_*_tip을 별도 RigidBody(FixedJoint)로 등록.
    #   Isaac Lab ContactSensor를 rl_dg_*_tip에 직접 부착 가능.
    #   USD sensor_spec 준수: force_limit_N=30.0 포화는 _sensor_max_contact_force에서 clamp(max=30.0)로 적용.
    ...
    cup_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=[0.5, 0.0, 0.38],   # 초기 USD 위치 (실제 spawn은 _reset_idx에서 결정)
            rot=[1.0, 0.0, 0.0, 0.0],
        ),
        spawn=UsdFileCfg(
