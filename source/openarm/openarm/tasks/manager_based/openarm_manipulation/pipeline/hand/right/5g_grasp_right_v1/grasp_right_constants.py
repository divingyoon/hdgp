...
# FK 검증 결과 (scripts/tools/openarm_fk.py, 캘리브레이션: Q_REF=[0.5,0.5,-0.6,0.7,0,0,1.0]):
#   j4=0.6 → palm_center ≈ (0.327, -0.407, 0.301) → z=0.301m (table+5cm, 파지 적정 높이)
#   j4=0.8 → palm_center ≈ (0.342, -0.420, 0.365) → z=0.365m (table+11cm, 너무 높음)
#   j4=1.3 → palm_center ≈ (0.324, -0.435, 0.557) → z=0.557m (table+31cm, 더 나쁨)
#   결론: j4 증가 → z 증가 (이전 세션의 /tmp/openarm_fk3.py는 방향이 반대였음)
# ---------------------------------------------------------------------------
ARM_START_POSE = [0.5, 0.1, 0.4, 0.6, -0.2, 0.0, 0.0]
