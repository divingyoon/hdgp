...
"""Lift-style right grasp environment configuration for 2g cup task.

This task keeps the original 2g_grasp_right_v1 scene spawn layout (robot/table/cup placement),
...
class Grasp2gSceneCfg(InteractiveSceneCfg):
    ...
    Spawn structure and placement are intentionally kept aligned with 2g_grasp_right_v1.
    """
...
class ActionsCfg:
    ...
    Left actions are preserved for compatibility with the hold-env wrapper,
    but are masked by curriculum_stage=1 so only right side moves.
    """
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        """Lift-style policy observations focused on right-hand grasp/lift."""

        joint_pos = ObsTerm(func=mdp.right_joint_pos_rel)
        joint_vel = ObsTerm(func=mdp.right_joint_vel_rel)
...
@configclass
class RewardsCfg:
    """Lift-style reward terms focused on right cup."""
    ...
    object_goal_tracking = RewTerm(
        ...
        params={"std": 0.4, "minimal_height": 0.04, "command_name": "object_pose", "object_cfg": SceneEntityCfg("cup2")},
        weight=20.0,
    )
...
@configclass
class Grasp2gRightV2EnvCfg(ManagerBasedRLEnvCfg):
    """Right-hand cup grasp/lift environment using lift-style learning design."""

    task_name: str = "grasp_2g_right_lift_style"
    debug_enabled: bool = True

    # Keep stage-masking behavior in Grasp2gHoldEnv:
    # stage 1 masks left arm/hand actions -> only right side is trainable.
