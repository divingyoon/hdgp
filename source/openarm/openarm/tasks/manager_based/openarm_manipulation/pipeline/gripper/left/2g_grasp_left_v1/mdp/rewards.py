...
def object_goal_distance_z_only(
    env: ManagerBasedRLEnv,
    std: float,
    minimal_height: float,
    command_name: str,
    ...
    """Reward the agent for tracking the goal z-position only using tanh-kernel."""
    robot: RigidObject = env.scene[robot_cfg.name]
    object: RigidObject = env.scene[object_cfg.name]
    command = env.command_manager.get_command(command_name)
    des_pos_b = command[:, :3]
    ...
    # Only compare z-axis
    z_distance = torch.abs(des_pos_w[:, 2] - object.data.root_pos_w[:, 2])
    return (object.data.root_pos_w[:, 2] > minimal_height) * (1 - torch.tanh(z_distance / std))
